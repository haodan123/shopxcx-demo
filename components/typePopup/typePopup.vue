<template>
	<view class="" @touchmove.stop.prevent="moveHandle">
		...
		<uni-transition @touchmove.stop.prevent="moveHandle" custom-class="popupBox" :mode-class="['fade', 'slide-top']"
			:duration="300" :show="show">
			...
			<view class="popupContent">
				...
				<view class="contentInfo">
					<view class="contentItem" v-for="(item,index) in 10" :key="index" @click="tapType(item,index)">
						<view class="image">
							<image src="https://cdn.uviewui.com/uview/empty/list.png" mode=""></image>
						</view>
						<text :class="{topActive:index==active}" class="text">蔬菜豆制品</text>
					</view>
				</view>
...
<script setup>
	...
	const emit = defineEmits(['close','useType'])
	const close = () => {
		emit('close')
	}
</script>
